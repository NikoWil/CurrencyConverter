...
function display_invalid_currency_error(from: string, to: string, date: Date) {
  display_text(
    `Error: the conversion from "${from}" to "${to}" is not defined for date "${date}"`,
  );
}

function display_converted_amount(
  from: string,
  to: string,
  amount: number,
  converted: number,
...
async function get_conversion_rate() {
  ...
  if (
    !from_elem ||
    !(from_elem instanceof HTMLInputElement) ||
    !to_elem ||
    !(to_elem instanceof HTMLInputElement)
  ) {
...
export async function convert_currencies() {
